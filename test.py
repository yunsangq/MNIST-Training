...
class MLP:
    def __init__(self):
        ...
        self.weights = [np.random.rand(y, x) for x, y in zip(NN[:-1], NN[1:])]
        self.epoch_accuracy = []
    ...
    def train(self, x, y):
        ...
        # Backpropagation
        delta = self.cost(activations[-1], y) * self.dsigmoid(vectors_z[-1])
        delta_b[-1] = delta
        delta_w[-1] = np.dot(delta, activations[-2].transpose())

        for i in xrange(2, 3):
            z = vectors_z[-i]
            delta = np.dot(self.weights[-i+1].transpose(), delta) * self.dsigmoid(z)
            delta_b[-i] = delta
