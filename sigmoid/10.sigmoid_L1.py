...
class MLP:
    ...
    def update(self, mini_batch, learn_rate, lmbda, n):
        ...
        self.weights = [w - learn_rate * (lmbda / n) * np.sign(w) - (learn_rate / len(mini_batch)) * bw
                        for w, bw in zip(self.weights, batch_w)]
        self.biases = [b - learn_rate * (lmbda / n) * np.sign(b) - (learn_rate / len(mini_batch)) * bb
                       for b, bb in zip(self.biases, batch_b)]
    ...
    def stochastic(self, train_data, epochs, mini_batch_size, learning_rate, valid_data, test_data, lmbda):
        ...
        self.save("sigmoid_L1.json", accuracy)
