...
class MLP:
    ...
    def update(self, mini_batch, learn_rate, lmbda, n):
        ...
        self.weights = [(1 - learn_rate * (lmbda / n)) * w - (learn_rate / len(mini_batch)) * bw
                        for w, bw in zip(self.weights, batch_w)]
        self.biases = [(1 - learn_rate * (lmbda / n)) * b - (learn_rate / len(mini_batch)) * bb
    ...
    def stochastic(self, train_data, epochs, mini_batch_size, learning_rate, valid_data, test_data, lmbda):
        ...
        self.save("sigmoid_L2_weight.json", accuracy)
