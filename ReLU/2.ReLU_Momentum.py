...
def err_disp(epochs, train, valid):
    ...
    fig.canvas.set_window_title('ReLU_Momentum_Cost per Epoch')
...
def acc_disp(epochs, train, valid):
    fig = plt.figure(facecolor='white')
    fig.canvas.set_window_title('ReLU_Momentum_Accuracy per Epoch')
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(range(epochs), train, color='#1F77B4', label='Training')
    ax.plot(range(epochs), valid, color='#b41f1f', label='Validation')
    ax.set_xlabel('Epochs')
...
class MLP:
    ...
    def feed_forward(self, inputs):
        ...
        for b, w in zip(self.biases, self.weights):
            if relu_check == 0:
                inputs = self.ReLU(np.dot(w, inputs) + b)
                relu_check = 1
            else:
    ...
    def train(self, x, y):
        ...
        for i in xrange(2, 3):
            # hiddendelta
            delta = np.dot(self.weights[-i+1].transpose(), delta) * self.dReLU(activations[-i])
            delta_b[-i] = delta
    ...
    def stochastic(self, train_data, epochs, mini_batch_size, learning_rate, valid_data, test_data, momentum):
        ...
        self.save("ReLU_Momentum.json", accuracy)
